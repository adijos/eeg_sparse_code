...
num_pre_data_files = 20; % # of data files to train on

% Variable Sizes
N = 16; % # of electrodes
M = 16; % # of dictionary elements
w = 401; % length of basis windows
T = 1000; % length of data samples
  ...
  for i = 1:num_pre_data_files
      ...
      for i = 1:batch_size
        ...
        while isempty(a);
          ...
          % Extract random 2.5 second eeg clip

          index = randi(size(full_data, 2) - 500 * 2);
          data = full_data(:, index:index + 500 * 2 - 1);
          T = size(data, 2);
