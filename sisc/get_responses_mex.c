...
int* get_active_set(const double* corr, int n, double beta, int *num_active)
{
  ...
  int *active_set = mxCalloc(num_nonzero, sizeof(int));
...
int* get_active_set_graft(const double* S_real, const double* corr, int n, double beta, int *num_active)
{
  ...
  int i,j,curr;
  int* active_set_zero;
  int num_nonzero, num_active_zero;
  int* active_set_all;
  mwSize dims[2];
  ...
  int num_elems = patch_M*patch_N*num_bases;
...
double* get_quadratic(const int *active_set, int num_active)
{
  ...
  double *fs_A = mxCalloc(num_active*num_active, sizeof(double));
...
void get_responses()
{
  ...
  for (iter=1; iter<=max_iter; iter++){
    ...
    /* Get the residual. */
    mwSize dims[3] = {patch_M, patch_N, num_channels};
    mxArray* err = mxCreateNumericArray(3, dims, mxDOUBLE_CLASS, mxCOMPLEX);
    double *err_real, *err_imag;
    get_real_imag(&err_real, &err_imag, err);
    for (chan = 0; chan < num_channels; chan++){
      for (elt = 0; elt < patch_M*patch_N; elt++){
        int ind = chan*patch_M*patch_N + elt;
    ...
    mxArray* corr = get_correlation(A_freq, err);
    ...
    mxArray* fs_x_init = mxCreateDoubleMatrix(num_active,1,mxREAL);
    mxArray* fs_b = mxCreateDoubleMatrix(num_active,1,mxREAL);
    for (i=0; i<num_active; i++){
    ...
    double *fs_A_ = get_quadratic(active_set, num_active);
    mxArray *fs_A = mxCreateDoubleMatrix(num_active, num_active, mxREAL);
    ...
    mxArray* fs_x = solve_fs(fs_A,fs_b,fs_x_init,beta,num_active);
  ...
  if (compute_stats){
    ...
    /* Compute the reconstruction. */
    mxArray* rec = get_reconstruction(A_freq, S);
    double *rec_real = mxGetPr(rec);
    
    /* Get the residual. */
    mwSize dims[3] = {patch_M, patch_N, num_channels};
    mxArray* err = mxCreateNumericArray(3, dims, mxDOUBLE_CLASS, mxCOMPLEX);
    double *err_real = mxGetPr(err);
    int chan, elt;
    for (chan = 0; chan < num_channels; chan++){
      for (elt = 0; elt < patch_M*patch_N; elt++){
        int ind = chan*patch_M*patch_N + elt;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  max_iter = round(mxGetScalar(prhs[3]));
  
  beta = mxGetScalar(prhs[4]);

  exact = round(mxGetScalar(prhs[5]));

  num_coords = round(mxGetScalar(prhs[6]));

  verbosity = round(mxGetScalar(prhs[7]));
